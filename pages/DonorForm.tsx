...
import { ChevronLeft, Save, FileText, Activity, Calculator, Info, Plus, Trash2, AlertTriangle } from 'lucide-react';
...
// --- Estilos y Componentes Auxiliares (Definidos FUERA del componente principal) ---
...
export const DonorForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.email && !emailRegex.test(formData.email)) {
        ...
        // Scroll to top roughly where email is
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-16 px-4 sm:px-6">
      ...
      <div className="flex items-center gap-4 border-b border-gray-200 pb-6 pt-4">
        ...
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 flex items-center gap-3">
            ...
            Registro Clínico de Donadora
          </h1>
          <p className="text-base text-slate-500 mt-1">Formato oficial de ingreso al Banco de Leche Humana.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white shadow-md rounded-2xl border border-slate-200 p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
             <InputGroup>
                ...
                <input type="date" className={`${inputClass} bg-slate-100 text-slate-600 font-semibold`} readOnly value={new Date().toISOString().split('T')[0]} />
             </InputGroup>
             <InputGroup>
             ...
             <InputGroup>
              ...
              <InputGroup>
                ...
                <input 
                    ...
                    onChange={handleChange} 
                    className={`${inputClass} ${errors.email ? 'border-red-500 focus:ring-red-500 text-red-900' : ''}`}
                    required 
                />
        ...
        <div className="bg-red-50 shadow-md rounded-2xl border border-red-200 p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <label className="flex items-start gap-4 p-4 bg-white rounded-lg border border-red-100 hover:border-red-300 transition-colors cursor-pointer">
                    <input 
                        type="checkbox" 
                        checked={formData.toxicSubstances}
                        onChange={(e) => handleCheckboxChange('toxicSubstances', e.target.checked)}
                ...
                <label className="flex items-start gap-4 p-4 bg-white rounded-lg border border-red-100 hover:border-red-300 transition-colors cursor-pointer">
                    <input 
                        type="checkbox" 
                        checked={formData.chemicalExposure}
                        onChange={(e) => handleCheckboxChange('chemicalExposure', e.target.checked)}
                        className="mt-1 h-5 w-5 text-red-600 rounded border-gray-300 focus:ring-red-500"
                    />
                    <div>
                ...
                <label className="flex items-start gap-4 p-4 bg-white rounded-lg border border-red-100 hover:border-red-300 transition-colors cursor-pointer">
                    <input 
                        type="checkbox" 
                        checked={formData.recentVaccines}
                        onChange={(e) => handleCheckboxChange('recentVaccines', e.target.checked)}
                        className="mt-1 h-5 w-5 text-red-600 rounded border-gray-300 focus:ring-red-500"
                    />
                    <div>
                ...
                <label className="flex items-start gap-4 p-4 bg-white rounded-lg border border-red-100 hover:border-red-300 transition-colors cursor-pointer">
                    <input 
                        type="checkbox" 
                        checked={formData.bloodTransfusionRisk}
                        onChange={(e) => handleCheckboxChange('bloodTransfusionRisk', e.target.checked)}
                        className="mt-1 h-5 w-5 text-red-600 rounded border-gray-300 focus:ring-red-500"
                    />
                    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <InputGroup className="md:col-span-3 flex items-end gap-6 bg-slate-50 p-4 rounded-xl border border-slate-200">
                <div className="flex items-center h-12">
                   <input 
                      type="checkbox" 
                      id="infantHospitalized" 
                      checked={formData.infantHospitalized} 
                      onChange={(e) => handleCheckboxChange('infantHospitalized', e.target.checked)}
                ...
                <div className="flex-1">
                   ...
                   <input 
                      name="infantHospitalService" 
                      value={formData.infantHospitalService} 
                      onChange={handleChange} 
                      disabled={!formData.infantHospitalized}
                      className={`${inputClass} disabled:opacity-50 disabled:bg-slate-200`}
                      placeholder={formData.infantHospitalized ? "Ej. UCIN, Cuneros..." : "N/A"}
                   />
                </div>
            </InputGroup>
            ...
            <InputGroup>
                ...
                <div className={`flex items-center justify-between p-3 rounded-lg border text-base font-bold shadow-sm ${
                   (formData.bmi || 0) < 18.5 ? 'bg-amber-50 text-amber-800 border-amber-200' :
                   (formData.bmi || 0) > 30 ? 'bg-orange-50 text-orange-800 border-orange-200' :
             ...
             <InputGroup className="md:col-span-4 flex flex-col md:flex-row items-start gap-6 p-5 bg-slate-50 rounded-xl border border-slate-200">
                <div className="min-w-[180px] pt-2">
                   <label className="flex items-center gap-3 cursor-pointer">
                      <input 
                        type="checkbox" 
                        checked={formData.pregnancyInfections} 
                        onChange={(e) => handleCheckboxChange('pregnancyInfections', e.target.checked)}
                        className={checkboxClass}
                      />
        ...
        {/* ANTECEDENTES PATOLÓGICOS PERSONALES DE LA MADRE */}
        <div className="bg-white shadow-md rounded-2xl border border-slate-200 p-8">
                        ...
                        {formData.pathologies?.map((pathology, idx) => (
                            <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                                <td className="px-6 py-4 text-base font-medium text-slate-900">
                                    {pathology.name === 'Otros' ? (
                                        <span className="italic text-slate-500 font-normal">Otros</span>
                                    ) : pathology.name}
                                </td>
                                <td className="px-6 py-4 text-center">
                                    <input 
                                        type="checkbox" 
                                        checked={pathology.present}
                                        onChange={(e) => handlePathologyChange(idx, 'present', e.target.checked)}
                                        className={checkboxClass}
                                    />
                                </td>
                                <td className="px-6 py-3">
                                    <input 
                                        type="text" 
                                        value={pathology.specification || ''}
                                        onChange={(e) => handlePathologyChange(idx, 'specification', e.target.value)}
                                        disabled={!pathology.present}
                                        className={tableInputClass}
                                        placeholder={pathology.name === 'Otros' ? 'Especifique...' : ''}
                                    />
                                </td>
                                <td className="px-6 py-3">
                                    <input 
                                        type="text" 
                                        value={pathology.timeElapsed || ''}
                                        onChange={(e) => handlePathologyChange(idx, 'timeElapsed', e.target.value)}
                                        disabled={!pathology.present}
                                        className={tableInputClass}
                                    />
                                </td>
                ...
                <div className="flex items-center gap-8 ml-4">
                     <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                            ...
                            checked={formData.isTakingMedication === true} 
                            onChange={() => setFormData(prev => ({ ...prev, isTakingMedication: true }))}
                     ...
                     <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                            type="radio" 
                            name="isTakingMedication" 
                            checked={formData.isTakingMedication === false} 
                            onChange={() => setFormData(prev => ({ ...prev, isTakingMedication: false }))}
                            className="h-5 w-5 text-pink-600 focus:ring-pink-500"
                        />
            ...
            {formData.isTakingMedication && (
                <div className="overflow-hidden rounded-xl border border-slate-200">
                            ...
                            {formData.medicationsList?.map((med, idx) => (
                                <tr key={idx}>
                                    ...
                                    <td className="px-3 py-3">
                                        <input 
                                            value={med.reason} 
                                            onChange={(e) => handleMedicationChange(idx, 'reason', e.target.value)}
                                            className={tableInputClass}
                                        />
                                    </td>
                                    <td className="px-3 py-3">
                                    ...
                                    <td className="px-3 py-3 text-center">
                                        {formData.medicationsList!.length > 1 && (
                                            <button type="button" onClick={() => removeMedicationRow(idx)} className="p-2 text-slate-400 hover:text-red-600 transition-colors">
                    ...
                    <div className="bg-slate-50 p-3 border-t border-slate-200">
                         <Button type="button" variant="outline" onClick={addMedicationRow} className="text-sm border-slate-300 text-slate-700">
        ...
        <div className="bg-white shadow-md rounded-2xl border border-slate-200 p-8">
            ...
            <div className="grid grid-cols-4 gap-6 mb-8 text-center bg-slate-50 p-6 rounded-xl border border-slate-200">
                  <InputGroup>
                     <Label>Gestas</Label>
                     <input type="number" name="gestations" value={formData.gestations} onChange={handleChange} className={`${inputClass} text-center font-bold text-lg`} />
                  </InputGroup>
                   <InputGroup>
                     <Label>Partos</Label>
                     <input type="number" name="deliveries" value={formData.deliveries} onChange={handleChange} className={`${inputClass} text-center font-bold text-lg`} />
                  </InputGroup>
                   <InputGroup>
                     <Label>Cesáreas</Label>
                     <input type="number" name="cesareans" value={formData.cesareans} onChange={handleChange} className={`${inputClass} text-center font-bold text-lg`} />
                  </InputGroup>
                   <InputGroup>
                     <Label>Abortos</Label>
                     <input type="number" name="abortions" value={formData.abortions} onChange={handleChange} className={`${inputClass} text-center font-bold text-lg`} />
                  </InputGroup>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="grid grid-cols-2 gap-6">
                    <InputGroup>
                        ...
                        <input type="number" name="sexualPartners" value={formData.sexualPartners} onChange={handleChange} className={inputClass} />
                    </InputGroup>
                    <InputGroup>
                        <Label>Método Anticonceptivo</Label>
                        <input name="contraceptiveMethod" value={formData.contraceptiveMethod} onChange={handleChange} className={inputClass} />
                    </InputGroup>
                </div>
                <InputGroup>
                    <Label>Anomalías</Label>
                    <input name="anomalies" value={formData.anomalies} onChange={handleChange} className={inputClass} placeholder="Especificar..." />
                </InputGroup>
            </div>
        </div>
                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                        {formData.labTests?.map((test, idx) => (
                            <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                <td className="px-4 py-3 font-semibold text-slate-900 text-base">
                                    {test.name === 'Otros' ? (
                                        <input placeholder="Especifique otro..." className="w-full bg-transparent border-b border-gray-300 focus:border-pink-500 outline-none placeholder-[#9A9A9A] text-[#333333]" />
                                    ) : test.name}
                                </td>
                                {/* Stages Columns */}
                                    ...
                                    <td key={stage} className="px-2 py-3 border-l border-slate-200 align-top">
                                        <div className="flex flex-col gap-2 p-1">
                                            <label className="flex items-center justify-center gap-2 cursor-pointer mb-1 bg-slate-50 p-1 rounded hover:bg-slate-200 transition-colors">
                                                <input 
                                                    type="checkbox"
                                                    checked={(test as any)[stage].performed}
                                                    onChange={(e) => handleLabChange(idx, stage as any, 'performed', e.target.checked)}
                                                    className={checkboxClass}
                                                />
                                            ...
                                            {(test as any)[stage].performed && (
                                                <div className="space-y-2 animate-in fade-in slide-in-from-top-1">
                                                    <input 
                                                        type="date"
                                                        value={(test as any)[stage].date}
                                                        onChange={(e) => handleLabChange(idx, stage as any, 'date', e.target.value)}
                                                        className="w-full text-xs border border-[#C6C6C6] rounded px-2 py-1 bg-[#F2F4F7] text-[#333333]"
                                                    />
                                                    <input 
                                                        type="text"
                                                        ...
                                                        className="w-full text-xs border border-[#C6C6C6] rounded px-2 py-1 bg-[#F2F4F7] text-[#333333] placeholder-[#9A9A9A]"
                                                    />
                                                </div>
                                            )}
                                        </div>
                                ...
                                <td className="px-4 py-3 border-l border-slate-200">
                                     <input 
                                        type="date"
                                        value={test.lastResultDate}
                                        onChange={(e) => handleLabChange(idx, 'root', 'lastResultDate', e.target.value)}
                                        className={tableInputClass}
                                    />
                                </td>
                            </tr>
        ...
        <div className="bg-white shadow-md rounded-2xl border border-slate-200 p-8">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {formData.donationType === 'REJECTED' && (
                    <InputGroup>
                        <Label required>Motivo de Rechazo</Label>
                        <input name="rejectionReason" value={formData.rejectionReason} onChange={handleChange} className={`${inputClass} border-red-300 bg-red-50 text-red-900`} placeholder="Especificar causa..." />
                    </InputGroup>
                )}
        ...
        <div className="bg-slate-50 rounded-2xl border border-slate-200 p-8">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <InputGroup>
                  <Label>Entrevista Realizada Por:</Label>
                  <input name="interviewerName" value={formData.interviewerName} onChange={handleChange} className={`${inputClass} bg-white`} />
               </InputGroup>
               <InputGroup>
                  <Label>Elaboró (Captura en Sistema):</Label>
                  <input name="elaboratorName" value={formData.elaboratorName} readOnly className={`${inputClass} bg-slate-200 text-slate-500`} />
               </InputGroup>
           </div>
        ...
        <div className="flex justify-end gap-6 pt-6 border-t border-slate-200">
           <Button type="button" variant="outline" onClick={() => navigate('/donors')} className="w-40 text-base py-3">
              Cancelar
           </Button>
